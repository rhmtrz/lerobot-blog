---
title: "LeRobot Primer — Environment, Glossary, Logs, and Tips Before You Start"
emoji: "📚"
type: "tech"
...
| `Mismatch between calibration values in the motor and the calibration file` | The motor's stored zero point doesn't match your saved file. Either accept the saved file (Enter) or re-run calibration (`c`). |
...
- **`robot.id` mismatch.** Calibration files are looked up by `--robot.id`. If teleop uses a different id than the one you calibrated with, it will keep prompting you to recalibrate.
